    ...
    var body: some View {
        ...
        .toolbar{
            ToolbarItem(placement: .navigationBarTrailing){
                Button(action: {
                    scrapVM.moveDataToOtherCategory(data, from: currentCategoryId, to: selectedCategoryId)
                    scrapVM.modifyCategoryOfData(userID: userVM.userIndex, linkID: data.linkId!, categoryId: selectedCategoryId) //📡 자료의 카테고리 이동 서버 통신
                    isShowMovingCategoryView.toggle()
                }) {
                    Text("저장")
                        .fontWeight(.semibold)
